...
public partial class TvObjectArrayEditor : TvEditorBase
{
    ...
    private async Task OnChangeData(int index, string key, object data)
    {
        if (Data != null && Data.ElementAt(index) != null)
        {
            var item = Data.ElementAt(index);
            var dataType = item?.GetType();
            dataType?.GetProperty(key)?.SetValue(item, data);
        }
